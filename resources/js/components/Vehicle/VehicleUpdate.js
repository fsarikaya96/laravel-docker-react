import React, {Component} from "react";
import axios from "axios";
import {withUrlParams} from "../urlParams";
import swal from "sweetalert";
import {Link} from "react-router-dom";

class VehicleUpdate extends Component {
    constructor(props) {
        super(props);
    }

    state = {
        plate: '',
        model: '',
        error_list: [],
    }
    handleInput = (e) => {
        this.setState({
            [e.target.name]: e.target.value
        });
    }

    /**
     * Vehicle and Driver By ID
     * @returns {Promise<void>}
     */
    async componentDidMount() {
        const { id } = this.props.params;
        const res = await axios.get(`http://localhost:8080/api/vehicle-edit/${id}`);

        if (res.data.success === true){
            this.setState({
                plate: res.data.message.plate,
                model: res.data.message.model,
            });
        }else {
            await swal({
                title: "Başarısız",
                text: res.data.message,
                icon: "warning",
                button: "Tamam",
            });
            window.location.replace('http://localhost:8080/vehicle');
        }
    }

    /**
     * Passenger Update
     * @param e
     * @returns {Promise<void>}
     */
    updateVehicle = async (e) => {
        e.preventDefault();
        document.getElementById('updateBtn').disabled = true
        document.getElementById('updateBtn').innerText = "Kaydediliyor..";
        const { id } = this.props.params;
        const res = await axios.put(`http://localhost:8080/api/vehicle-update/${id}`, this.state);
        if (res.data.success === true) {
            await swal({
                title: "Başarılı",
                text: res.data.message,
                icon: "success",
                button: "Tamam",
            });
            window.location.replace('http://localhost:8080/vehicle');
        } else {
            document.getElementById('updateBtn').disabled = false
            document.getElementById('updateBtn').innerText = "Güncelle";
            this.setState({
                error_list: res.data.errors
            });
        }
    }

    render() {
        return (
            <div className="container py-3">
                <div className="row">
                    <div className="col-md-12">
                        <h2 className="text-center mb-3">Araç Sayfası</h2>
                        <hr className="mb-4"/>
                        <div className="row justify-content-center">
                            <div className="col-md-6">
                                <div className="card card-outline-secondary">
                                    <div className="card-header">
                                        <h3 className="mb-0">Araç Güncelle
                                            <Link to="/vehicle" className="btn btn-primary btn-sm float-end">Geri</Link>
                                        </h3>
                                    </div>
                                    <div className="card-body">
                                        <form onSubmit={this.updateVehicle}>
                                            <div className="form-group">
                                                <label htmlFor="plate">Plaka</label>
                                                <input className="form-control" onChange={this.handleInput}
                                                       value={this.state.plate} id="plate"
                                                       name="plate" type="text"/>
                                                <span className="text-danger">{this.state.error_list.plate}</span>
                                            </div>
                                            <div className="form-group">
                                                <label htmlFor="model">Araç Modeli</label>
                                                <input className="form-control" onChange={this.handleInput}
                                                       value={this.state.model} id="model"
                                                       name="model" type="text"/>
                                                <span className="text-danger">{this.state.error_list.model}</span>
                                            </div>
                                            <button className="btn btn-success mt-3 btn-block" style={{width: "100%"}}
                                                    id="updateBtn"  type="submit">Güncelle
                                            </button>

                                        </form>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        );
    }
}

export default withUrlParams(VehicleUpdate);
